...
#=========[TOMO] 20250413 pronptを変更（状況・原因・対策・結果）====================
def generate_summary(text: str):
    try:
        prompt = f"""
        以下の会議内容をもとに、次の形式で300文字程度ずつに要約・分類してください。
        ①【会議の要約】  
        会議全体の内容を、300文字程度で簡潔にまとめてください。

        ②【知見】  
        会議の中で得られた複数の知見（学び、発見）を、それぞれ以下の形式で箇条書きしてください。  
        必ず「- タイトル:」「内容:」という形式で、1つの知見につき1ブロックずつ出力してください。  
        ...
        ③【悩み】  
        議論の中で挙がった複数の悩み・課題を、それぞれ以下の形式で箇条書きしてください。  
        こちらも「- タイトル:」「内容:」という形式で出力してください。  
